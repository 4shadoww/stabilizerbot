...
class YunoModule:
	...
	def addToList(self, rev):
		text = self.api.getText(config["list_path"])
		newpage = "* [["+rev["title"]+"]]"
		if(newpage in text):
			return
		text += "\n"+newpage
		logger.info("adding page "+rev["title"]+" to stabilize list")
		self.api.savePage(text, (config_loader.dictionary[config_loader.cur_conf["core"]["lang"]]["editsum"]["LS"] % rev["title"]), minor=True)
	...
	def run(self, rev):
		...
		if(len(stable_log["query"]["logevents"]) >= self.config["stabilizes"]):
			if(not config_loader.cur_conf["test"]):
				self.addToList(rev)
